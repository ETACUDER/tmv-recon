...
# Bill number = the EZee receipt token as-is (REC-####). No derived FY prefix —
# the accountant wants exactly what EZee shows, nothing added.
...
def parse_sales_detail_orders(path: str | Path) -> list[dict]:
    ...
    for r in rows:
        ...
        if oid and cur:
            ...
            if net > 0.005:
                orders.append({"date": cur, "rec": rec, "inv": rec,
                               "net": round(net, 2)})
    return orders
